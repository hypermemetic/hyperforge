...
pub struct RepoOp {
    pub repo: Repo,
    pub op: SyncOp,
    /// What differs: e.g. ["description", "visibility", "3 commits ahead"]
...
fn norm_desc(d: &Option<String>) -> &str {
    d.as_deref().unwrap_or("")
}

/// Return the list of fields that differ between two repos.
/// Empty vec means repos are in sync.
...
mod tests {
    ...
    async fn test_diff_update_needed() {
        ...
        let update_op = diff.ops.iter().find(|o| o.op == SyncOp::Update).unwrap();
        assert_eq!(update_op.details, vec!["description"]);
    }
