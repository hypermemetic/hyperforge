...
mod tests {
    ...
    fn test_forge_repo_to_observed_converts_all_fields() {
        ...
        let api_repo = ForgeRepo {
            ...
            clone_url: "https://github.com/myorg/my-repo.git".to_string(),
            ssh_url: "git@github.com:myorg/my-repo.git".to_string(),
        };
        ...
        assert!(observed.exists_on(&Forge::GitHub));
        let state = observed.forge_states.iter()
            .find(|s| s.forge == Forge::GitHub)
            .expect("GitHub state should exist");

        assert!(state.exists);
        assert_eq!(state.url, Some("https://github.com/myorg/my-repo.git".to_string()));
        assert_eq!(state.visibility, Some(Visibility::Private));
        assert_eq!(state.description, Some("A test repository".to_string()));
        // forge_id is None for GitHub adapter (API doesn't return simple ID)
    ...
    fn test_forge_repo_to_observed_handles_none_description() {
        ...
        let api_repo = ForgeRepo {
            ...
            clone_url: "https://github.com/org/minimal-repo.git".to_string(),
            ssh_url: "git@github.com:org/minimal-repo.git".to_string(),
        };
    ...
    #[test]
    fn test_all_bridge_errors_map_to_forge() {
        ...
        let errors_with_expected_forge: Vec<(BridgeForgeError, Option<Forge>)> = vec![
            (BridgeForgeError::AuthenticationFailed { message: "x".to_string() }, Some(Forge::GitHub)),
            (BridgeForgeError::TokenExpired { forge: Forge::GitHub, message: "x".to_string() }, Some(Forge::GitHub)),
            (BridgeForgeError::Forbidden { message: "x".to_string() }, Some(Forge::GitHub)),
            (BridgeForgeError::RateLimited { retry_after: Duration::from_secs(1) }, Some(Forge::GitHub)),
            ...
            (BridgeForgeError::ApiError { status: 400, message: "x".to_string() }, Some(Forge::GitHub)),
            (BridgeForgeError::ServerError { status: 500, message: "x".to_string() }, Some(Forge::GitHub)),
        ];
