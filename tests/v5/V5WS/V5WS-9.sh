...
# V5WS-9 uses local workspace fixtures (ws_with_one_repo, ws_cross_org,
# ws_empty) for structural test cases. Tokens for the embedded cred
# refs come from tier2.env (HF_TIER2_GITHUB_TOKEN / _CODEBERG_TOKEN).
# If tier-2 config is absent the script SKIPs cleanly.
hf_require_tier2

gh_token="${HF_TIER2_GITHUB_TOKEN:-missing}"
cb_token="${HF_TIER2_CODEBERG_TOKEN:-missing}"
...
hf_put_secret "secrets://gh-token" "$gh_token" || true
...
hf_put_secret "secrets://gh-token" "$gh_token" || true
hf_put_secret "secrets://cb-token" "$cb_token" || true
...
hf_load_fixture "ws_cross_org"
hf_put_secret "secrets://gh-token" "$gh_token" || true
# codeberg cred intentionally NOT seeded
out=$(hf_cmd workspaces sync name=multi)
