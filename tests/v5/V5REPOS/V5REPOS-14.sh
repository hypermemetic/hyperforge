...
snap=$(hf_cmd repos sync --org "$HF_TIER2_GITHUB_ORG" --name "$HF_TIER2_GITHUB_REPO")
...
out=$(hf_cmd repos push --org "$HF_TIER2_GITHUB_ORG" --name "$HF_TIER2_GITHUB_REPO")
echo "$out" | hf_assert_event '(.type == "push_remote_ok" or .type == "push_summary") and (.url == "'$url'" or (.results // [] | map(.url) | index("'$url'")))'
verify=$(hf_cmd repos sync --org "$HF_TIER2_GITHUB_ORG" --name "$HF_TIER2_GITHUB_REPO")
...
python3 - "$HF_CONFIG/orgs/${HF_TIER2_GITHUB_ORG}.yaml" "dry-$STAMP" <<'PY'
import sys, yaml
...
PY
out=$(hf_cmd repos push --org "$HF_TIER2_GITHUB_ORG" --name "$HF_TIER2_GITHUB_REPO" --dry_run true)
echo "$out" | hf_assert_event '(.type == "push_remote_ok" or .type == "push_summary")'
after=$(hf_cmd repos sync --org "$HF_TIER2_GITHUB_ORG" --name "$HF_TIER2_GITHUB_REPO")
...
PY
set +e
out=$(hf_cmd repos push --org "$HF_TIER2_GITHUB_ORG" --name "$HF_TIER2_GITHUB_REPO" 2>&1)
rc=$?
set -e
...
open(p, "w").write(yaml.safe_dump(d))
PY
out=$(hf_cmd repos push --org "$HF_TIER2_GITHUB_ORG" --name "$HF_TIER2_GITHUB_REPO" --remote "$url")
echo "$out" | hf_assert_count '(.type == "push_remote_ok" or .type == "push_remote_error")' 1

# --- restore original forge-side description ---
python3 - "$HF_CONFIG/orgs/${HF_TIER2_GITHUB_ORG}.yaml" "$original" <<'PY'
import sys, yaml
p, v = sys.argv[1], sys.argv[2]
d = yaml.safe_load(open(p))
d["repos"][0].setdefault("metadata", {})["description"] = v
open(p, "w").write(yaml.safe_dump(d))
PY
hf_cmd repos push --org "$HF_TIER2_GITHUB_ORG" --name "$HF_TIER2_GITHUB_REPO" --remote "$url" >/dev/null

# --- no token leakage ---
full=$(hf_cmd repos push --org "$HF_TIER2_GITHUB_ORG" --name "$HF_TIER2_GITHUB_REPO" --remote "$url" 2>&1)
